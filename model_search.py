...
from operations import WSAvgPool2d, WSDilConv, WSMaxPool2d, WSReLUConvBN, WSBN, WSSepConv, FactorizedReduce, MaybeCalibrateSize, AuxHeadCIFAR, apply_drop_path
    ...
    def forward(self, x, x_id, x_op, y, y_id, y_op, step, bn_train=False):
        ...
        if y_op == 0:
            ...
            if stride>1:
                assert stride == 2
                if y_id == 0:
                    y = self.y_id_reduce_1(y, bn_train=bn_train)
                else:
                    y = self.y_id_reduce_2(y, bn_train=bn_train)
        elif y_op == 3:
...
class NASWSNetworkCIFAR(nn.Module):
    ...
    def forward(self, input, arch, step=None, bn_train=False):
        aux_logits = None
        conv_arch, reduc_arch = arch[0],arch[1]
